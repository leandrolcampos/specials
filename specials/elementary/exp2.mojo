...
fn _exp2_impl[
    ...
    """Implements the base-2 exponential function based on reference papers."""
    alias ln2: SIMD[dtype, simd_width] = 0.69314_71805_59945_30941_72321
    ...
    @parameter
    if dtype == DType.float32:
        alias ln2: SIMD[dtype, simd_width] = bitcast[dtype, DType.uint32](
            0x3F31_7218,
        )
        ...
        let xn = math.round(safe_x)
        let xf = safe_x - xn

        let yn = math.round(xf * 32.0)
        let yn2 = math.mod(yn, 32.0)
        let yn1 = yn - yn2

        let y_reduced_lead = math.fma(-yn, ln2_over_32_lead, xf * ln2)
        let y_reduced_trail = -yn * ln2_over_32_trail

        index = yn2.cast[DType.int32]()
        exponent = (xn + yn1 / 32).cast[DType.int32]()

        let y_reduced = y_reduced_lead + y_reduced_trail

        expm1_r = y_reduced_lead + (
            math.fma(y_reduced * y_reduced, polynomial(y_reduced), y_reduced_trail)
        )

    else:  # dtype == DType.float64
        alias ln2: SIMD[dtype, simd_width] = bitcast[dtype, DType.uint64](
            0x3FE62E42_FEFA39EF,
        )
