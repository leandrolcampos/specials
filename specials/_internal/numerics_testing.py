...
def py_relative_error(result, truth):
    ...
    of that difference and ratio are done in the next higher precision compared to the
    working precision.

    The data types `result.dtype` and `truth.dtype` must correspond to the working
    precision and a higher precision floating-point types, respectively.

    Args:
        result: A NumPy array of values whose deviation to assess.
        truth: A NumPy Array of values presumed correct. Must broadcast with `result`.

    Returns:
        A NumPy array of element-wise relative error values. The data type of the
        output array is the next higher precision floating-point type compared to
        the data type of `result`:
            - `np.float64` if `result.dtype` is `np.float32`; or
            - `np.longdouble` if `result.dtype` is `np.float64`.

    Raises:
        TypeError: If `result.dtype` is not a floating-point of single (`float32`) or
            double (`float64`) precision.
        TypeError: If `truth.dtype` is a floating-point type but does not have higher
            precision than `result.dtype`.
    """
    result = np.array(result)

    if result.dtype not in (np.float32, np.float64):
        raise TypeError(
            "`result.dtype` must be a floating-point of single (`float32`) or double "
            "(`float64`) precision"
        )
    ...
    if truth.dtype.kind == "f":
        if np.finfo(truth.dtype).precision <= np.finfo(result.dtype).precision:
            raise TypeError(
                "When `truth.dtype` is a floating-point type, it must have higher "
                "precision than `result.dtype`"
            )
    ...
    safe_denominator = np.where(
        ...
        np.ones_like(truth),
        np.where(
            (result > 0) & (truth > 0) & truth_is_subnormal, largest_subnormal, truth
        ),
    )
...
def py_kahan_ulp(x, target_dtype):
    """Computes the Kahan-ulp function at `x` element-wise.

    Ulp stands for "unit in the last place".

    If `x` is an arbitrary real number, the Kahan-ulp at `x` is the distance between the
    two floating-point numbers nearest `x`, even if `x` is not contained in the interval
    between them.

    The computation of the Kahan-ulp is done in the next higher precision compared to the
    working precision.

    Args:
        x: Array of floating-point values. The data type of this array must have higher
            precision than `target_dtype`.
        target_dtype: Data type corresponding to the working precision.

    Returns:
        Array of element-wise Kahan-ulp values. The data type of the output array is the
        next higher precision floating-point type compared to the data type of `x`:
            - `np.float64` if `result.dtype` is `np.float32`; or
            - `np.longdouble` if `result.dtype` is `np.float64`.

    Raises:
        TypeError: If `target_dtype` is not a floating-point of single (`float32`) or
            double (`float64`) precision.
        TypeError: If `x.dtype` is a floating-point type but does not have higher
            precision than `target_dtype`.
    """
    target_dtype = np.dtype(target_dtype)
    if target_dtype not in (np.float32, np.float64):
        raise TypeError(
            "`target_dtype` must be a floating-point of single (`float32`) or double "
            "(`float64`) precision"
        )
    ...
    exponent = np.where(
        safe_x / powermin <= one + np.ldexp(one, -(t + 2)), exponent - 1, exponent
    )
...
def py_error_in_ulps(result, truth):
    ...
    The error in ulps is defined as `abs((result - truth) / kahan_ulp(truth))`. The
    computation of that difference and ratio are done in the next higher precision
    ...
    Returns:
        A NumPy array of element-wise error in ulps values. The data type of the
        output array is the next higher precision floating-point type compared to
        the data type of `result`:
            - `np.float64` if `result.dtype` is `np.float32`; or
            - `np.longdouble` if `result.dtype` is `np.float64`.
    ...
    ulp_truth = py_kahan_ulp(truth, target_dtype=target_dtype)
