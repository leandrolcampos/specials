...
from test_utils import numerics_testing
...
def test_relative_error(result_dtype, truth_dtype):
    ...
    actual = numerics_testing.relative_error(result, truth)
...
def test_relative_error_invalid_result_dtype():
    ...
    with pytest.raises(TypeError, match="single .* or double"):
        _ = numerics_testing.relative_error(result, truth)


def test_relative_error_invalid_truth_dtype():
...
def test_kahan_ulp(x, target_dtype, expected):
    output_dtype = numerics_testing._promote_dtype(target_dtype)
    actual = numerics_testing.kahan_ulp(x, target_dtype)

    np.testing.assert_equal(actual.dtype, output_dtype)
    np.testing.assert_allclose(actual, expected, rtol=1e-15, atol=0.0)


def test_kahan_ulp_invalid_x_dtype():
    x = np.array([0.0, 1.0], dtype=np.float32)
    target_dtype = np.float32

    with pytest.raises(TypeError, match="higher precision"):
        numerics_testing.kahan_ulp(x, target_dtype)


def test_kahan_ulp_invalid_target_dtype():
...
def test_error_in_ulps(result, truth, expected):
    output_dtype = numerics_testing._promote_dtype(result.dtype)
    actual = numerics_testing.error_in_ulps(result, truth)

    np.testing.assert_equal(actual.dtype, output_dtype)
    np.testing.assert_allclose(actual, expected, rtol=1e-15, atol=0.0)


def test_error_in_ulps_invalid_result_dtype():
    result = np.int32(1)
    truth = np.float64(1.0)

    with pytest.raises(TypeError, match="single .* or double"):
        numerics_testing.error_in_ulps(result, truth)


def test_error_in_ulps_invalid_truth_dtype():
